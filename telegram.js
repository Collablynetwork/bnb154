...
function getRuntimeSettingsPath() {
  return (
    config.runtimeSettingsPath ||
    path.join(config.storageDir || path.join(__dirname, "storage"), "runtime-settings.json")
  );
}
...
function loadRuntimeSettings() {
  ...
  try {
    ...
    return {
      strategyRetentionHours: Number(parsed.strategyRetentionHours || 4),
      updatedAt: parsed.updatedAt || null,
    };
  } catch (error) {
    console.error("loadRuntimeSettings error:", error.message);
    return {
      strategyRetentionHours: 4,
      updatedAt: null,
    };
  ...
  return {
    reply_markup: {
      keyboard: [
        ...
        [BUTTONS.RETENTION, BUTTONS.HELP, BUTTONS.MENU],
      ],
...
function registerHandlers(bot, callbacks) {
  ...
  async function actionSignals(msg) {
    ...
    if (!open.length) {
      await sendWithMenu(bot, msg.chat.id, "No active monitored trades.");
      return;
    }
    ...
    await sendWithMenu(bot, msg.chat.id, `📡 Monitored Trades\n${text}`);
  }
